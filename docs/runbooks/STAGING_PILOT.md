...
- 2026-05-15: Owner-approved staging schema update applied `0003_inventory_module.sql` only to non-PM staging project `mgdsvapgltzwhsioccqd`. `npm run test:staging-schema-public` now covers the inventory tables and `fulfillment_stock_feed` view with zero-row checks. No production migration, stock seed/import, PM read/write, provider mutation, label, export, or tracking action was run.
